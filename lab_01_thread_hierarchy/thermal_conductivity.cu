...
void step_kernel_mod(int ni, int nj, float fact, float* temp_in, float* temp_out)
{
  ...
  for ( int j = y_idx; j < nj-1; j += y_stride) {
    for ( int i = x_idx; i < ni-1; i += x_stride) {
...
int main()
{
  ...
  int block_x = (ni + threads_per_block.x - 1)/threads_per_block.x;
  int block_y = (nj +  threads_per_block.y - 1)/ threads_per_block.y;
